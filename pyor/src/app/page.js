...
const Page = () => {
  ...
  useEffect(() => {
    ...
    if (newdata.length === 0) {
      axios
        .get(`https://api.coingecko.com/api/v3/coins/${coin}/market_chart?vs_currency=usd&days=365&interval=daily`)
        .then((res) => {
          let sum = [];
          let count = 0;
          let val = 0;
    ...
    <div className='text-red-500 font-bold bg-black pt-5'>
      <div className='flex justify-center '>
        ...
        >
          {search &&
            search.map((item) => (
              <div key={item}  className='flex text-white border-solid border-2 rounded-md h-8 mt-1 '>
              <h1
                className={`text-white font-xs sm:text-base align-middle p-1 uppercase sm:text-{10px} `}
              >
                {item}
              </h1>
                <div onClick={()=>handleremove(item)}><RxCross2 /></div>
              </div>
            ))}
        </div>
      </div>     
        ...
        {data &&
          data.map((item) => (
            <div key={item.id} onClick={(e) => setcoin(item.id)}>
              <h1 className='text-blue-500 font-bold uppercase p-5'>{item.id}</h1>
              <hr />
            </div>
          ))}
      </div>
